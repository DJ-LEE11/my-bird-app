...
typedef void (^SSLivenessNormalCompletion) (NSDictionary *images, id faceInfo, NSUInteger remindCode);
typedef void (^SSDetectStrategyCompletion) (id faceinfo,NSDictionary *images, NSUInteger remindCode);
...
typedef NS_ENUM(NSInteger, BDFaceCompletionStatus) {
    ...
    BDFaceCompletionStatusNeedFaceLibError = -21,    // 缺少视频组件库
};
...
// 活体检测状态，LivenessRemindCode
- (void)livenessActionDidFinishWithCode:(NSUInteger)code;

// 人脸识别检测，DetectRemindCode
- (void)detectionActionDidFinishWithCode:(NSUInteger)code;
...
@property (nonatomic, assign) BOOL willFinishRecording;

@property (nonatomic, assign) BOOL willStartRecording;

@property (nonatomic, strong, readonly) id faceBaseInstance;
...
- (void)connectPreviewLayer:(id)pLayer;
/**
 * 创建用于实名认证的参数
 */
...
- (void)faceLogicWithCaptureOutput:(UIImage *)image sampleBuffer:(NSDictionary *)sampleBuffer fromConnection:(id)connection;
