...
@optional
- (BOOL)sapiWebView:(SAPIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(WKNavigationType)navigationType;
...
@interface SAPIWebView : PASSWebView
 ...
 */
@property (nonatomic, copy) BOOL(^externalPageHandler)(NSDictionary *userInfo);
...
*  标识帐号切换里的三方登录场景
*/
@property (nonatomic, copy) NSString *sceneFrom;
...
/**
 ...
 @param params 透传参数
 */
- (void)loadLoginWithType:(SAPILoginType)type extraParams:(NSDictionary *)params DEPRECATED_MSG_ATTRIBUTE("Use loadLoginWithConfig:extraParams: instead, this property will be removed in a future release and should be avoided");
...
/**
 ...
 @param params 透传参数。键值为中文或特殊字符时需要URL Encode后传入。
 */
- (void)loadAccountDetectWithBduss:(NSString *)bduss extraParams:(NSDictionary *)params DEPRECATED_MSG_ATTRIBUTE("this method will be removed in a future release");
...
DEPRECATED_MSG_ATTRIBUTE("this method will be removed in a future release");
...
/**
 *  加载绑定控件。绑定手机、换绑手机、绑定邮箱、换绑邮箱、解绑邮箱都可以通过该方法调用
 *
...
@interface SAPIWebView (SAPIWebViewShippingAddition)
/**
 加载地址选择页面
 
 @param params 额外的参数，没有时传 nil 即可。
 */
- (void)loadShippingAddressWithBduss:(NSString *)bduss extraParams:(NSDictionary *)params;
